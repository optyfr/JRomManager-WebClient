...
public class BatchTrrntChkPanel extends VLayout {
    ...
    private final class SdrGrid extends ListGrid {
        private SdrGrid() {
            ...
            updateOrAdditem.setDynamicTitleFunction((target, menu, item) -> Client.getSession()
                    .getMsg(sdr.getSelectedRecords().length == 1 ? "BatchToolsTrrntChkPanel.mntmUpdTorrent.text" : "BatchToolsTrrntChkPanel.mntmAddTorrent.text"));
    ...
    public BatchTrrntChkPanel() {
        ...
        detectArchivedFolderItem.addChangedHandler(
                e -> Client.sendMsg(JsonUtils.stringify(Q_Global.SetProperty.instantiate().setProperty("trntchk.detect_archived_folders", (Boolean) e.getValue()))));
        ...
        removeUnknownFilesItem.addChangedHandler(
                e -> Client.sendMsg(JsonUtils.stringify(Q_Global.SetProperty.instantiate().setProperty("trntchk.remove_unknown_files", (Boolean) e.getValue()))));
        ...
        removeWrongSizedItem.addChangedHandler(
                e -> Client.sendMsg(JsonUtils.stringify(Q_Global.SetProperty.instantiate().setProperty("trntchk.remove_wrong_sized_files", (Boolean) e.getValue()))));
