    ...
    class Systems extends ListGrid // NOSONAR
    {
        ...
        public Systems() {
            ...
            submenu1.addItem(new MenuItem(Client.getSession().getMsg("MainFrame.mntmAllBios.text")) {
                {
                    addClickHandler(event -> ProfileViewer.reset(() -> systems.selectRecords(
                            Stream.of(systems.getRecords()).filter(r -> r.getAttribute("type").equals("BIOS")).collect(Collectors.toList()).toArray(new ListGridRecord[0]))));
                }
            });
            submenu1.addItem(new MenuItem(Client.getSession().getMsg("MainFrame.mntmAllSoftwares.text")) {
                {
                    addClickHandler(event -> ProfileViewer.reset(() -> systems.selectRecords(Stream.of(systems.getRecords())
                            .filter(r -> r.getAttribute("type").equals("SOFTWARELIST")).collect(Collectors.toList()).toArray(new ListGridRecord[0]))));
                }
            });
            ...
            submenu2.addItem(new MenuItem(Client.getSession().getMsg("MainFrame.mntmAllBios.text")) {
                {
                    addClickHandler(event -> ProfileViewer.reset(() -> systems.deselectRecords(
                            Stream.of(systems.getRecords()).filter(r -> r.getAttribute("type").equals("BIOS")).collect(Collectors.toList()).toArray(new ListGridRecord[0]))));
                }
            });
            submenu2.addItem(new MenuItem(Client.getSession().getMsg("MainFrame.mntmAllSoftwares.text")) {
                {
                    addClickHandler(event -> ProfileViewer.reset(() -> systems.deselectRecords(Stream.of(systems.getRecords())
                            .filter(r -> r.getAttribute("type").equals("SOFTWARELIST")).collect(Collectors.toList()).toArray(new ListGridRecord[0]))));
                }
            });
                    ...
                    addItem(new MenuItem(Client.getSession().getMsg("MainFrame.mntmInvertSelection.text")) {
                        {
                            addClickHandler(event -> ProfileViewer.reset(() -> {
                                ...
                                ListGridRecord[] to_select = Stream.of(sources.getRecords()).filter(r -> !to_unselect_list.contains(r)).collect(Collectors.toList())
