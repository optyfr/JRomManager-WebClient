	...
	public SettingsDebugPanel()
	{
		...
		setItems(
			new SelectItem("cbDbgLevel") {{
				setColSpan(2);
				setTitle(Client.getSession().getMsg("MainFrame.lblLogLevel.text"));
				...
				setDefaultValue(Client.getSession().getSetting(fname2name.get(getName()), "INFO"));
			}},
			new TextItem("txtDbgMemory", Client.getSession().getMsg("MainFrame.lblMemory.text")) {{
				setWidth("*");
				setCanEdit(false);
				setShouldSaveValue(false);
			}},
			new ButtonItem() {{
				setTitle(Client.getSession().getMsg("MainFrame.btnGc.text"));
				setShouldSaveValue(false);
				setStartRow(false);
				setWidth(150);
