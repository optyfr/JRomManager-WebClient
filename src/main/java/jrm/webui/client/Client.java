...
public class Client implements EntryPoint {
        ...
        private void execute(A_ a) {
            switch (a.getCmd()) {
                ...
                case "Progress.setInfos":
                    ...
                    break;
                case "Progress.extendInfos":
                    mainWindow.update(new A_Progress.ExtendInfos(a));
                    break;
    ...
    private void lpr(boolean init) {
        ...
        request.setUseSimpleHttp(true); // obligatoire car sinon on s'adresse à un serveur rpc smartclient, et ce n'est
                                        // pas le cas ici
        request.setHttpMethod("GET"); // on simplifie le plus possible la requête (POST est plus complexe pour le
                                      // protocole HTTP et nécessite 2 aller-retour)
    ...
    public static void sendMsg(String msg) {
        ...
        request.setUseSimpleHttp(true); // obligatoire car sinon on s'adresse à un serveur rpc smartclient, et ce n'est
                                        // pas le cas ici
        request.setHttpMethod("POST"); // on simplifie le plus possible la requête (POST est plus complexe pour le
                                       // protocole HTTP et nécessite 2 aller-retour)
        request.setData(msg);
